...
const GeolocationSvc = Cc['@mozilla.org/geolocation;1'].getService(Ci.nsISupports);
...
var GeolocationClass = Class({
   ...
   */
  get lastPosition() { return GeolocationSvc.lastPosition; },
  ...
  /**
   * Gets a one time current position, this requires the allowed preference to be set true
   * @see https://developer.mozilla.org/en-US/docs/XPCOM_Interface_Reference/NsIDOMGeoGeolocation#getCurrentPosition%28%29
   */
  getCurrentPosition : function getCurrentPosition() {
    var options = namespace(this).options;
    var { promise, resolve, reject } = defer();

    if (this.allowed) {
      GeolocationSvc.getCurrentPosition( position => {
  ...
  stopWatching : function stopWatching() {
    if (this.isWatching()) {
      GeolocationSvc.clearWatch(namespace(this).watchID);
   ...
   * @see https://developer.mozilla.org/en-US/docs/XPCOM_Interface_Reference/NsIDOMGeoGeolocation#watchPosition%28%29
   */
  watchPosition : function watchPosition() {
    ...
    if (this.allowed) { 
      if (!this.isWatching()) {
        try {
          namespace(this).watchID = GeolocationSvc.watchPosition( position => {
            this._setposition(position);
            resolve(this);
          },
          error => {
